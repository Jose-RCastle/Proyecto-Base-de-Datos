...
router.get('/', async (_req, res, next) => {
  try {
    const [resumen, activas, recientes, pagos] = await Promise.all([
      pool.query(`SELECT COUNT(*) AS total_espacios, COUNT(*) FILTER (WHERE estado = 'OCUPADO') AS ocupados, COUNT(*) FILTER (WHERE estado = 'DISPONIBLE') AS disponibles, ROUND(COUNT(*) FILTER (WHERE estado = 'OCUPADO') * 100.0 / NULLIF(COUNT(*), 0), 2) AS porcentaje_ocupacion, COALESCE((SELECT SUM(monto) FROM pagos WHERE fecha_pago::date=CURRENT_DATE), 0) AS ingresos_hoy FROM espacios`),

...
router.get('/ingresos-fecha', consulta(`SELECT DATE(fecha_pago) AS fecha, COUNT(*) AS cantidad_pagos, SUM(monto) AS ingresos_totales FROM pagos GROUP BY DATE(fecha_pago) ORDER BY fecha DESC`));

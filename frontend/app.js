...
async function salida(){const [active,methods]=await Promise.all([get('/estadias/activas'),get('/metodos-pago')]);content.innerHTML=`<div class="split"><article class="panel"><h2>Estadías activas</h2><div class="active-list">${active.length?active.map(x=>`<button class="active-stay" data-id="${x.estadia_id}"><b>#${x.estadia_id} · ${esc(x.placa)}</b><small>${esc(x.cliente)} · ${esc(x.zona)} / ${esc(x.espacio)}<br>Entrada: ${date(x.fecha_entrada)}</small></button>`).join(''):'<p class="empty">No hay vehículos estacionados.</p>'}</div></article><form class="form-card" id="exit"><h2>Registrar salida</h2><div id="exitSummary"><p class="empty">Seleccione una estadía activa para ver el resumen.</p></div><input type="hidden" name="estadia_id" required><label>Método de pago</label><select name="metodo_pago" required><option value="">Seleccione un método</option>${methods.map(m=>`<option>${esc(m.nombre)}</option>`).join('')}</select><p>La salida se registrará con la hora actual por PostgreSQL.</p><button class="submit">Finalizar estadía y registrar pago</button></form></div>`;document.querySelectorAll('.active-stay').forEach(b=>b.onclick=()=>{const x=active.find(a=>a.estadia_id==b.dataset.id);document.querySelector('[name=estadia_id]').value=x.estadia_id;document.querySelector('#exitSummary').innerHTML=`<dl class="summary"><dt>Vehículo</dt><dd>${esc(x.placa)} · ${esc(x.tipo_vehiculo)}</dd><dt>Espacio</dt><dd>${esc(x.zona)} · ${esc(x.espacio)}</dd><dt>Entrada</dt><dd>${date(x.fecha_entrada)}</dd><dt>Tarifa</dt><dd>${money(x.precio_hora)} / hora</dd></dl>`});formSubmit('#exit','/salida','POST',data=>{notice(`Estadía #${data.estadia_id} finalizada. Total: ${money(data.total)}.`);content.insertAdjacentHTML('beforeend',ticket(data,true))})}
async function clientForm(x={}){content.innerHTML=`<form class="form-card" id="clientForm"><h2>${x.cliente_id?'Editar':'Nuevo'} cliente</h2><label>Nombre</label><input name="nombre" required value="${esc(x.nombre||'')}"><label>Apellido</label><input name="apellido" required value="${esc(x.apellido||'')}"><label>Teléfono</label><input name="telefono" value="${esc(x.telefono||'')}"><label>Correo</label><input type="email" name="correo" value="${esc(x.correo||'')}"><button class="submit">Guardar cliente</button><button type="button" class="secondary" onclick="show('clientes')">Cancelar</button></form>`;formSubmit('#clientForm',`/clientes${x.cliente_id?'/'+x.cliente_id:''}`,x.cliente_id?'PUT':'POST',()=>show('clientes'))}
async function vehicleForm(x={}){const [clients,types]=await Promise.all([get('/clientes'),get('/tipos-vehiculo')]);content.innerHTML=`<form class="form-card" id="vehicleForm"><h2>${x.vehiculo_id?'Editar':'Registrar'} vehículo</h2><p>El cliente es opcional.</p><label>Placa</label><input name="placa" maxlength="15" required value="${esc(x.placa||'')}"><label>Cliente (opcional)</label><select name="cliente_id"><option value="">Sin cliente</option>${clients.map(c=>`<option value="${c.cliente_id}" ${x.cliente_id==c.cliente_id?'selected':''}>${esc(c.nombre)} ${esc(c.apellido)}</option>`).join('')}</select><button type="button" class="link-button" id="quickClient">+ Crear cliente nuevo</button><label>Tipo de vehículo</label><select name="tipo_vehiculo_id" required><option value="">Seleccione tipo</option>${types.map(t=>`<option value="${t.tipo_vehiculo_id}" ${x.tipo_vehiculo_id==t.tipo_vehiculo_id?'selected':''}>${esc(t.nombre)}</option>`).join('')}</select><label>Marca</label><input name="marca" value="${esc(x.marca||'')}"><label>Modelo</label><input name="modelo" value="${esc(x.modelo||'')}"><label>Año</label><input type="number" name="anio" min="1900" max="2100" value="${esc(x.anio||'')}"><button class="submit">Guardar vehículo</button><button type="button" class="secondary" onclick="show('vehiculos')">Cancelar</button></form>`;document.querySelector('#quickClient').onclick=()=>clientForm();formSubmit('#vehicleForm',`/vehiculos${x.vehiculo_id?'/'+x.vehiculo_id:''}`,x.vehiculo_id?'PUT':'POST',()=>show('vehiculos'))}
async function spaceForm(x={}){const [zones,types]=await Promise.all([get('/zonas'),get('/tipos-vehiculo')]);content.innerHTML=`<form class="form-card" id="spaceForm"><h2>${x.espacio_id?'Editar':'Crear'} espacio</h2><p>El estado inicia disponible y solo cambia con entrada/salida.</p><label>Código</label><input name="codigo" required maxlength="10" value="${esc(x.codigo||'')}"><label>Zona</label><select name="zona_id" required>${zones.map(z=>`<option value="${z.zona_id}" ${x.zona_id==z.zona_id?'selected':''}>${esc(z.nombre)}</option>`).join('')}</select><label>Tipo de vehículo</label><select name="tipo_vehiculo_id" required>${types.map(t=>`<option value="${t.tipo_vehiculo_id}" ${x.tipo_vehiculo_id==t.tipo_vehiculo_id?'selected':''}>${esc(t.nombre)}</option>`).join('')}</select><button class="submit">Guardar espacio</button><button type="button" class="secondary" onclick="show('espacios')">Cancelar</button></form>`;formSubmit('#spaceForm',`/espacios${x.espacio_id?'/'+x.espacio_id:''}`,x.espacio_id?'PUT':'POST',()=>show('espacios'))}
async function clients(){const rows=await get('/clientes');content.innerHTML=`<article class="panel"><div class="panel-title"><h2>Clientes</h2><button class="submit" id="add">+ Nuevo cliente</button></div>${table(['Nombre','Teléfono','Correo','Vehículos',''],rows.map(x=>`<tr><td>${esc(x.nombre)} ${esc(x.apellido)}</td><td>${esc(x.telefono)}</td><td>${esc(x.correo)}</td><td><button class="link-button detail" data-id="${x.cliente_id}">Ver</button></td><td><button class="link-button edit" data-id="${x.cliente_id}">Editar</button></td></tr>`))}</article>`;document.querySelector('#add').onclick=()=>clientForm();document.querySelectorAll('.edit').forEach(b=>b.onclick=()=>clientForm(rows.find(x=>x.cliente_id==b.dataset.id)));document.querySelectorAll('.detail').forEach(b=>b.onclick=async()=>{const x=await get('/clientes/'+b.dataset.id);notice(`Vehículos de ${x.nombre} ${x.apellido}: ${x.vehiculos.length?x.vehiculos.map(v=>v.placa).join(', '):'ninguno'}`)})}
async function vehicles(){const rows=await get('/vehiculos');content.innerHTML=`<article class="panel"><div class="panel-title"><h2>Vehículos</h2><button class="submit" id="add">+ Registrar vehículo</button></div>${table(['Placa','Cliente','Tipo','Marca / modelo','Año',''],rows.map(x=>`<tr><td><b>${esc(x.placa)}</b></td><td>${esc(x.cliente)}</td><td>${esc(x.tipo_vehiculo)}</td><td>${esc(x.marca)} ${esc(x.modelo)}</td><td>${esc(x.anio)}</td><td><button class="link-button edit" data-id="${x.vehiculo_id}">Editar</button></td></tr>`))}</article>`;document.querySelector('#add').onclick=()=>vehicleForm();document.querySelectorAll('.edit').forEach(b=>b.onclick=()=>vehicleForm(rows.find(x=>x.vehiculo_id==b.dataset.id)))}
async function spaces(){const rows=await get('/espacios');content.innerHTML=`<article class="panel"><div class="panel-title"><h2>Espacios</h2><button class="submit" id="add">+ Crear espacio</button></div>${table(['Código','Zona','Tipo','Estado',''],rows.map(x=>`<tr><td><b>${esc(x.codigo)}</b></td><td>${esc(x.zona)}</td><td>${esc(x.tipo_vehiculo)}</td><td>${badge(x.estado)}</td><td><button class="link-button edit" data-id="${x.espacio_id}">Editar</button></td></tr>`))}</article>`;document.querySelector('#add').onclick=()=>spaceForm();document.querySelectorAll('.edit').forEach(b=>b.onclick=()=>spaceForm(rows.find(x=>x.espacio_id==b.dataset.id)))}
